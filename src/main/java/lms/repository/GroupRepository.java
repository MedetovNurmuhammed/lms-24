...
public interface GroupRepository extends JpaRepository<Group, Long> {
    ...
    @Query("""
                ...
                from Group g where g.trash.id is null order by g.dateOfStart asc
            """)
    Page<GroupResponse> findAllGroup(Pageable pageable);

    @Query("select g.id from Group g join g.courses c join c.instructors i where i.id = :id order by g.dateOfStart asc")
    ...
    @Query("select new lms.dto.response.GroupWithoutPagination(g.id, g.title) from Group g where g.trash is null order by g.dateOfStart asc")
    List<GroupWithoutPagination> findAllGroupsWithoutTrash();

    @Query("select s from Group s where s.id in (:ids) order by s.dateOfStart asc")
